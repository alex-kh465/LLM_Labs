...
def main():
    ...
    if st.session_state.question and st.button("🚀 Get Answers", type="primary"):
        ...
        with st.spinner("🧠 Processing your question in all languages..."):
            ...
            # Malayalam Answer
            if st.session_state.pdf_texts.get('Malayalam'):
                try:
                    answers['malayalam'] = answer_malayalam(question, st.session_state.pdf_texts['Malayalam'])
                except Exception as e:
                    answers['malayalam'] = f"Error: {e}"
            else:
                answers['malayalam'] = "Malayalam PDF not available"
    ...
    if st.session_state.show_answers and st.session_state.answers:
        ...
        with col2:
            st.markdown('<div class="language-section">', unsafe_allow_html=True)
            st.subheader("🇮🇳 Malayalam Answer")

            malayalam_answer = st.session_state.answers.get('malayalam', '')
            if malayalam_answer:
                st.markdown(f'<div class="answer-box"><strong>Answer:</strong> {malayalam_answer}</div>', unsafe_allow_html=True)

                # Generate TTS for Malayalam
                if st.button("🔊 Generate Malayalam Audio", key="ml_audio"):
                    with st.spinner("Generating speech..."):
                        try:
                            from gtts import gTTS
                            import tempfile
                            ...
                            # Generate TTS
                            tts = gTTS(text=malayalam_answer, lang='ml')
                            tts.save(temp_path)
                            ...
                            st.audio(audio_bytes, format='audio/mp3')
                            st.success("🎵 Malayalam audio generated!")
                ...
                st.warning("⚠️ Malayalam answer not available")
